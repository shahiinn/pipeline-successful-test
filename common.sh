...
docker_exec() {
    if [[ "${LOCAL_RUN}" = "true" ]]; then
        ...
        # shellcheck disable=SC2086
        docker exec --interactive -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -e KUBECONFIG=/kubeconfig.yaml -e HELM_HOST=127.0.0.1:44134 -e HELM_TILLER_SILENT=true ct "$@"
    fi
}
